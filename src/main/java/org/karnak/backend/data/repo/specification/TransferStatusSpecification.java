...
public class TransferStatusSpecification implements Specification<TransferStatusEntity> {
  ...
  public Predicate toPredicate(
      Root<TransferStatusEntity> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
    // Predicates to fill
    ...
    if (transferStatusFilter != null) {
      ...
      // Sop Instance Uid
      buildCriteriaSopInstanceUid(
          criteriaBuilder, predicates, pSopInstanceUidOriginal, pSopInstanceUidToSend);
      // Sent
  ...
  private void buildCriteriaStudyUid(
      ...
      Path<String> pStudyUidOriginal,
      Path<String> pStudyUidToSend) {
    if (transferStatusFilter.getStudyUid() != null
        && StringUtils.isNotBlank(transferStatusFilter.getStudyUid())) {
      predicates.add(
          criteriaBuilder.or(
              criteriaBuilder.like(
                  pStudyUidOriginal, LIKE + transferStatusFilter.getStudyUid().trim() + LIKE),
              criteriaBuilder.like(
                  pStudyUidToSend, LIKE + transferStatusFilter.getStudyUid().trim() + LIKE)));
    }
  }
  ...
  private void buildCriteriaSerieUid(
      CriteriaBuilder criteriaBuilder,
      List<Predicate> predicates,
      Path<String> pSerieUidOriginal,
      Path<String> pSerieUidToSend) {
    if (transferStatusFilter.getSerieUid() != null
        && StringUtils.isNotBlank(transferStatusFilter.getSerieUid())) {
      predicates.add(
          criteriaBuilder.or(
              criteriaBuilder.like(
                  pSerieUidOriginal, LIKE + transferStatusFilter.getSerieUid() + LIKE),
              criteriaBuilder.like(
                  pSerieUidToSend, LIKE + transferStatusFilter.getSerieUid() + LIKE)));
    }
  }
  ...
  private void buildCriteriaSopInstanceUid(
      CriteriaBuilder criteriaBuilder,
      List<Predicate> predicates,
      Path<String> pSopInstanceUidOriginal,
      Path<String> pSopInstanceUidToSend) {
    if (transferStatusFilter.getSopInstanceUid() != null
        && StringUtils.isNotBlank(transferStatusFilter.getSopInstanceUid())) {
      predicates.add(
          criteriaBuilder.or(
              criteriaBuilder.like(
                  pSopInstanceUidOriginal, LIKE + transferStatusFilter.getSopInstanceUid() + LIKE),
              criteriaBuilder.like(
                  pSopInstanceUidToSend, LIKE + transferStatusFilter.getSopInstanceUid() + LIKE)));
    }
  }
  ...
  private void buildCriteriaSent(
    ...
    if (transferStatusFilter.getTransferStatusType() != null
      ...
      predicates.add(
          criteriaBuilder.equal(pSent, transferStatusFilter.getTransferStatusType().getCode()));
    }
  }
  ...
  private void buildCriteriaTransferDate(
    ...
    if (transferStatusFilter.getStart() != null && transferStatusFilter.getEnd() == null) {
      predicates.add(
          criteriaBuilder.greaterThanOrEqualTo(pTransferDate, transferStatusFilter.getStart()));
    } else if (transferStatusFilter.getStart() == null && transferStatusFilter.getEnd() != null) {
      predicates.add(
          criteriaBuilder.lessThanOrEqualTo(pTransferDate, transferStatusFilter.getEnd()));
    } else if (transferStatusFilter.getStart() != null
        && transferStatusFilter.getEnd() != null
        && transferStatusFilter.getStart().isBefore(transferStatusFilter.getEnd())) {
      predicates.add(
          criteriaBuilder.between(
              pTransferDate, transferStatusFilter.getStart(), transferStatusFilter.getEnd()));
    }
  }
}
