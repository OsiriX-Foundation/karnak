 ...
 * orientation relative to the patient from the mathematical position and orientation
 * attributes, and including other descriptive attributes such as from dicom directory
 * records and images using multi-frame functional groups.
 *
 * <p>
 * C.7.6.1.1.1 Patient Orientation. The Patient Orientation (0020,0020) relative to the
 * image plane shall be specified by two values that designate the anatomical direction of
 * the positive row axis (left to right) and the positive column axis (top to bottom). The
 * first entry is the direction of the rows, given by the direction of the last pixel in
 * the first row from the first pixel in that row. The second entry is the direction of
 * the columns, given by the direction of the last pixel in the first column from the
 * first pixel in that column. Anatomical direction shall be designated by the capital
 * letters: A (anterior), P (posterior), R (right), L (left), H (head), F (foot). Each
 * value of the orientation attribute shall contain at least one of these characters. If
 * refinements in the orientation descriptions are to be specified, then they shall be
 * designated by one or two additional letters in each value. Within each value, the
 * letters shall be ordered with the principal orientation designated in the first
 * character.
 *
 * <p>
 * C.7.6.2.1.1 Image Position And Image Orientation. The Image Position (0020,0032)
 * specifies the x, y, and z coordinates of the upper left hand corner of the image; it is
 * the center of the first voxel transmitted. Image Orientation (0020,0037) specifies the
 ...
 * respectively followed by the Column value for the x, y, and z axes respectively. The
 * direction of the axes is defined fully by the patient's orientation. The x-axis is
 * increasing to the left hand side of the patient. The y-axis is increasing to the
 * posterior side of the patient. The z-axis is increasing toward the head of the patient.
 * The patient based coordinate system is a right handed system, i.e. the vector cross
 * product of a unit vector along the positive x-axis and a unit vector along the positive
 * y-axis is equal to a unit vector along the positive z-axis.
 *
...
public abstract class ImageOrientation {
	...
	/**
	 * Get a label describing the major axis from a unit vector (direction cosine) as
	 * found in ImageOrientationPatient.
	 *
	 * <p>
	 * Some degree of deviation from one of the standard orthogonal axes is allowed before
	 * deciding no major axis applies and returning null.
	 * @param x
	 * @param y
	 * @param z
	...
	/**
	 * Get a label describing the axial, coronal or sagittal plane from row and column
	 * unit vectors (direction cosines) as found in ImageOrientationPatient.
	 *
	 * <p>
	 * Some degree of deviation from one of the standard orthogonal planes is allowed
	 * before deciding the plane is OBLIQUE.
	 * @param rowX
	 * @param rowY
	 * @param rowZ
	 * @param colX
	 * @param colY
	 * @param colZ
	 * @return the string describing the plane of orientation, AXIAL, CORONAL, SAGITTAL or
	 * OBLIQUE
	 */
	...
	/**
	 * Get a PatientOrientation style string from a unit vector (direction cosine) as
	 * found in ImageOrientationPatient.
	 *
	 * <p>
	 * Returns letters representing R (right) or L (left), A (anterior) or P (posterior),
	 * F (feet) or H (head).
	 *
	 * <p>
	 * If the orientation is not precisely orthogonal to one of the major axes, more than
	 * one letter is returned, from major to minor axes, with up to three letters in the
	 * case of a "double oblique".
	 * @param x
	 * @param y
	 * @param z
	...
	/**
	 * Get a PatientOrientation style string from row and column unit vectors (direction
	 * cosines) as found in ImageOrientationPatient.
	 *
	 * <p>
	 * Returns letters representing R (right) or L (left), A (anterior) or P (posterior),
	 * F (feet) or H (head).
