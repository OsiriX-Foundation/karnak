...
public class DeIdentificationComponent extends VerticalLayout {
	...
	private void addComponents() {
		...
		deIdentificationDiv.add(disclaimerLabel, projectDropDown, deIdentificationName, pseudonymTypeSelect,
	...
	/**
	 * Build deidentification div which is visible or not depending on the activate
	 * deidentification checkbox
	 */
	 ...
	 * uncheck checkbox deidentification
	 */
	...
	public void setTextOnSelectionProject(ProjectEntity projectEntity) {
		if (projectEntity != null && projectEntity.getProfileEntity() != null) {
			deIdentificationName.setShowValue(String.format("The profile %s [version %s] will be used",
	...
	private void initDestinationBinder() {
		...
		destinationBinder.forField(projectDropDown)
				.withValidator(project -> project != null || (project == null && !deIdentificationCheckbox.getValue()),
						"Choose a project")
				.bind(DestinationEntity::getProjectEntity, DestinationEntity::setProjectEntity);

		destinationBinder.forField(pseudonymTypeSelect).withValidator(type -> type != null, "Choose pseudonym type\n")
				.bind(destination -> {
