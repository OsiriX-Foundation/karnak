...
public class Util {
  ...
  public static Future<Boolean> portIsOpen(
      final ExecutorService es, final String host, final int port, final int timeout) {
    return es.submit(
        ...
        if (netint != null) {
          ...
          if (mac_byte != null) {
            ...
            for (int i = 0; i < mac_byte.length; i++) {
              result.append(
                  String.format("%02X%s", mac_byte[i], (i < mac_byte.length - 1) ? "-" : ""));
            }
          }
  ...
  public static boolean getNetworkResponse(
      StringBuilder result, String aet, String host, int port, boolean fontIcon) {
  ...
  public static boolean getNetworkResponse(
    ...
    if (!reachable) {
      ...
      if (xml) {
        ...
        result.append(
            reachable
                ? "<span style=\"color:green\">" + getOKItem(fontIcon)
  ...
  public static boolean getEchoResponse(
              ...
              ConfigNode node =
                  new ConfigNode(
                      ...
                      new DicomNode(
                          trimSplit(line[1]),
                          trimSplit(line[2]),
                          Integer.parseInt(trimSplit(line[3]))));
              list.add(node);
            } catch (Exception e) {
