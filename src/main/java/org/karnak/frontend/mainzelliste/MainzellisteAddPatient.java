...
public class MainzellisteAddPatient extends VerticalLayout {
	...
	public void saveInMainzelliste() {
		binder.validate();
		if (binder.isValid()) {
			final Fields newPatientFields = new Fields(patientIdField.getValue(),
					...
					patientSexField.getValue(), issuerOfPatientIdField.getValue());

			try {
				final PseudonymApi pseudonymApi = new PseudonymApi();
				final String pseudonym = pseudonymApi.addExtID(newPatientFields, externalIdField.getValue());
				if (pseudonym != null) {
					final String strPatient = "ExternalID: " + externalIdField.getValue() + " " + "PatientID:"
							+ newPatientFields.get_patientID() + " " + "PatientName:"
							+ newPatientFields.get_patientName() + " " + "IssuerOfPatientID:"
